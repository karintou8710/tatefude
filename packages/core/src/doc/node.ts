...
class PlotType<Param = unknown> extends BaseType<Param> {
  ...
  /** 自分の content の先頭にキャレット位置があるか */
  get cursorAtContentStart(): boolean {
    return this.spec.cursorAtContentStart !== false;
  }
  /** 自分の content の末尾にキャレット位置があるか */
  get cursorAtContentEnd(): boolean {
    return this.spec.cursorAtContentEnd !== false;
  }
  /** 中身が空のときに CSS で出す代役の文字 */
...
export namespace Plot {
  ...
  export interface Spec<Param> extends Node.Spec<Param> {
    ...
    /**
     * 中身が空のときに出す代役の文字。**CSS の生成内容として出す**ので DOM のテキストには
     * 入らない。実ノードにするとバッファと DOM がずれ、変換にもクリップボードにも混ざる。
     */
    placeholder?: string;
  }
}
