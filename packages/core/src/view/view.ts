...
export class EditorView {
  ...
  constructor(
    ...
    this.dom.className = "tf-editor";
    this.dom.setAttribute("data-tf-editor", "");
  ...
  /** `tr` は購読者に渡すためだけのもの。省略しても state は同じように押し出される */
  updateState(state: EditorState, tr: Transaction | null = null): void {
    ...
    const update: ViewUpdate = {
      ...
      prevState,
      tr,
      docChanged: prevState.doc !== state.doc,
      selectionChanged: !prevState.selection.eq(state.selection),
    };
  ...
  /** 実行できるかだけ見る。ボタンの disabled 用 */
  can(command: Command): boolean {
    return command(this.state) !== false;
  }
  ...
  textblockForDOM(node: globalThis.Node | null): TextblockView | null {
    ...
    const blockDOM = element?.closest("[data-tf-textblock]") ?? null;
